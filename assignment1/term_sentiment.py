...
def assignment_output(diction):
    __function__ = 'assignment_output'
    for key in diction.keys():

        score = float( diction[key][0][0] - diction[key][0][1]) / diction[key][0][2]
        score = ("{0:.2f}".format(round(score,2)))
...
#                continue
            ...
            (tweetScore, calcWordScore) = \
                    get_sentiment_score(textArray, wordScores)
            
            msg = "Sentiment Score from file: " + str(tweetScore)
            test_output(msg,1,__function__,__file__,inspect.currentframe().f_lineno)
            ...
            if text == 'you':
                msg = "Tweet Text: " + text
                test_output(msg,1,__function__,__file__,inspect.currentframe().f_lineno)
            # Now have a tweet score and a list of unscored words.
            # set the pos and neg counts

            if tweetScore != 0:
                for data in calcWordScore:
                    word = data['word']
                    msg = "Word is: "+str(word)
                    test_output(msg,1,__function__,__file__,inspect.currentframe().f_lineno)

                    # check to see if the word alrady exists
                    if word in d.keys():
                        positive = d[word][0][0]
                        negative = d[word][0][1]
                        wordCount= d[word][0][2]
                        # found a word in the dict
                        msg = "Found key in dict: " + str (word) + \
                                "\nSeen this word "+str(wordCount)+" times."
                        output(msg,1,__function__,__file__)
                        msg = "Dict = " + str (d[word])
                        test_output(msg,1,__function__,__file__,inspect.currentframe().f_lineno)
                        ...
                        if tweetScore > 0:
                            ...
                            d[word] = [(positive, negative, count)]
                            msg = "Found a key.\nDict = " + str (d[word])
                            test_output(msg,1,__function__,__file__,inspect.currentframe().f_lineno)
                        elif tweetScore < 0:
                            wordCount = wordCount + 1
                        ...
                        # word is not in the dict
                        if tweetScore > 0:
                            values = (1,0,1) 
                            d.setdefault(word, []).append(values)
                            msg = "New key.\n"+"Dict = " + str (d[word])
                            test_output(msg,1,__function__,__file__,inspect.currentframe().f_lineno)
                        elif tweetScore < 0:
                            values = (0,1,1) 
        ...
        #if 'rt' in d.keys():
        msg = "Example word 'you': "+ str(d['you'])
        test_output(msg,1,__function__,__file__,inspect.currentframe().f_lineno)

#    if 'you' in d.keys():
#        youScore = float( d['you'][0][0] - d['you'][0][1]) / d['you'][0][2]
#        print str(d['you']) + " score is: " + str(youScore)

    assignment_output (d,count)
